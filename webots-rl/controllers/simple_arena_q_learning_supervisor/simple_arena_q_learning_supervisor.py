"""
Supervisor launcher for tabular Q-learning training or evaluation in the simple arena.

Overview:
    This script instantiates the Webots Supervisor, wraps it with an
    ``EnvironmentSimpleArena``, and optionally runs multiple Q-learning
    training episodes (epochs). In evaluation mode it runs a single episode
    without learning and logs the final state.

Execution Modes (env var ``TRAIN``):
    TRAIN=1   -> Training mode (runs ``EPOCHS`` Q-learning episodes, saves model).
...
from brain.trainer.simple_arena.q_learning import TrainerQLearningSimpleArena
from brain.utils.logger import logger
from controller import Supervisor

MODEL_NAME = "simple_arena_q_learning"
TIME_STEP = 64
EPISODE_SIZE = 3000
EPOCHS = 2500
ALPHA = 0.1
...
if __name__ == "__main__":
    ...
    if train:
        trainer = TrainerQLearningSimpleArena(
