...
class TrainerSarsa(TrainerTDTabular):
    ...
    def update_value_table(self, transition: TransitionSchema) -> dict[str, float]:
        ...
        if current.done:
            ...
            next_action = next_.action
            td_target = (
                current.reward
                + self.gamma * self.model.value_table[next_obs_index][next_action]
            )
        td_error = td_target - self.model.value_table[obs_index][current.action]
        self.model.value_table[obs_index][current.action] += self.alpha * td_error
        logger.debug(
            f"SARSA update: obs={obs_index} action={current.action} "
