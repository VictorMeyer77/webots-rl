...
from corl.trainer.algorithm.td_tabular import TrainerTDTabular
...
class TrainerDoubleQLearning(TrainerTDTabular):
    ...
    Extends :class:`~corl.trainer.algorithm.td_tabular.TrainerTDTabular` with
    ...
    def __init__(self, *args, **kwargs):
        ...
        :class:`~corl.trainer.algorithm.td_tabular.TrainerTDTabular`. After the
    ...
    def update_value_table(self, transition: TransitionSchema) -> dict[str, float]:
        ...
        q_old = q_update[obs_index][current.action]
        td_error = td_target - q_old
        q_update[obs_index][current.action] += self.alpha * td_error

        logger.debug(
            f"Double-Q update: obs={obs_index} action={current.action} "
