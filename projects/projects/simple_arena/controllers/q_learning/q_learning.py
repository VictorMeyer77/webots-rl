...
class EpuckQLearningController(Epuck):
    """
    E-puck controller driven by a Q-learning value-table policy.

    Discretises distance-sensor readings into bins and looks up the
    greedy action from a pre-trained Q-table.
    """
...
if __name__ == "__main__":
    ...
    if config.get("train_id") is not None:
        epuck = EpuckQLearningController(
            robot=robot,
            timestep=TIME_STEP,
            action_repeat=ACTION_REPEAT,
            model=None,
        ...
        model = ModelValueTable(
            model_dir="/Users/victormeyer/Dev/Self/webots-rl/projects/.train/mlflow/f15f149aca69478b82ba05de73fdae83/artifacts/model"
        )
